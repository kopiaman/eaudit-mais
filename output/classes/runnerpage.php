...
//		foreach ($this->googleMapCfg['mainMapIds'] as $mapId)
	...
	function createPerPage()
	{
		...
		for($i=0;$i<count($this->arrRecsPerPage);$i++)
		{
			if($this->arrRecsPerPage[$i]!=-1)
				...
				$rpp.= "<option value=\"-1\" ".($this->pageSize == $this->arrRecsPerPage[$i] ? "selected" : "").">"."Tunjuk semua"."</option>";
		}
	...
	{
		$message = ($this->is508 == true ? "<a name=\"skipdata\"></a>" : "")."Tiada rekod dijumpai";
	...
	function buildPagination() 
	{
		...
		else
		{
			...
			if($this->maxPages > 1) 
			{
				...
				if($counterstart != 1) 
				{
					$pagination.= $this->getPaginationLink(1,"Pertama")."&nbsp;:&nbsp;";
					$pagination.= $this->getPaginationLink($counterstart - 1,"Sebelum")."&nbsp;";
				}
				$pagination.= "[";
				...
				if($counterend != $this->maxPages) 
				{
					$pagination.= "&nbsp;".$this->getPaginationLink($counterend + 1,"Berikut")."&nbsp;:&nbsp;";
	...
	*/
	...
	{
		if( $page == "add" )
			return GetTableCaption($table).", "."Tambah rekod baru";
		if( $page == "edit" )
			return GetTableCaption($table).", "."Sunting rekod"." [". $this->getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "view" )
			return GetTableCaption($table).", "."Lihat rekod"." [". $this->getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "export" )
			return "Ekspot";
		if( $page == "import" )
			return GetTableCaption($table).", "."Import";
		if( $page == "search" )
			return GetTableCaption($table)." - "."Carian maju";
		if( $page == "print" )
		...
		if( $page == "register" )
			return "Daftar";
		if( $page == "changepwd" )
			return "Tukar katalaluan";
		if( $page == "remind" )
			return "Peringatan Katalaluan";	
		if( $page == "chart" )
			return GetTableCaption($table); 
		if( $page == "report" )
			return GetTableCaption($table);	
