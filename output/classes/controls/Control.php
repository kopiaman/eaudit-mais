...
class EditControl
{
	...
	function EditControl($field, $pageObject, $id, $connection)
	{
		...
		$this->searchOptions[CONTAINS] = "Mengandungi";
		$this->searchOptions[EQUALS] = "Bersamaan";
		$this->searchOptions[STARTS_WITH] = "Mula dengan";
		$this->searchOptions[MORE_THAN] = "Lebih daripada";
		$this->searchOptions[LESS_THAN] = "Kurang daripada";
		$this->searchOptions[BETWEEN] = "Di antara";
	...
	function readWebValue(&$avalues, &$blobfields, $legacy1, $legacy2, &$filename_values)
	{
		...
		if(!($this->webValue===false))
		{
			...
			if($this->field == "pass" && $this->pageObject->tName == "admin_users")
			{
				$this->webValue = md5($this->webValue);	
			}
