...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="user" && $field=="pass")
	{
		$value='Hidden';;
		return $value;
	}
				if($table=="audit_form" && $field=="aStatus" && $ptype=="list")
	{
		$status=$data['aStatus'];
if($status=="Draf"){
$value ="<font color ='red'>$status</font>";
}else if($status=="Lulus"){ 
$value ="<font color ='blue'>$status</font>";}
else if($status=="Draf-Pindaan"){ 
$value ="<font color ='red'><strong>$status</strong></font>";}
else if($status=="Selesai"){
$value ="<font color ='green'><strong>$status</strong></font>";
}else{

};;
		return $value;
	}
				if($table=="audit_form" && $field=="opt")
	{
		$value="";;
		return $value;
	}
				if($table=="admin_members" && $field=="pass")
	{
		$value='Hidden';;
		return $value;
	}
				if($table=="admin_users" && $field=="pass")
	{
		$value='Hidden';;
		return $value;
	}
				if($table=="report_SKT" && $field=="orga")
	{
		$value="";;
		return $value;
	}
				if($table=="report_susulan" && $field=="opt")
	{
		$value="";;
		return $value;
	}
				if($table=="audit_form_manual" && $field=="aStatus" && $ptype=="list")
	{
		$status=$data['aStatus'];
if($status=="Draf"){
$value ="<font color ='red'>$status</font>";
...
$value ="<font color ='green'><strong>$status</strong></font>";
		...
		return $value;
				...
				if($table=="audit_form_manual" && $field=="opt")
	{
		$value="";;
		return $value;
	}
...
function fileCustomExpression($file, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="audit_form" && $field=="attachment")
	{
		;
		return $value;
	}
				if($table=="audit_info" && $field=="attachment")
	{
		//$value =substr($file["usrName"],0,10);
$value =$file["usrName"];	;
		return $value;
	}
				if($table=="audit_info" && $field=="reply_attachment")
	{
		;
		return $value;
	}
				if($table=="audit_info" && $field=="reply_attachment2")
	{
		;
		return $value;
	}
				if($table=="audit_info" && $field=="reply_attachment3")
	{
		;
		return $value;
	}
				if($table=="web_file" && $field=="webAttachment")
	{
		;
		return $value;
	}
				if($table=="web_meeting" && $field=="webAttachment")
	{
		;
		return $value;
	}
				if($table=="audit_form_manual" && $field=="attachment")
	{
		;
		return $value;
	}
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="user" && $field=="seksyen")
	{
		return " CatID=4";
	}
				if($table=="user" && $field=="unit")
	{
		return "catID=5";
	}
				if($table=="audit_form" && $field=="aStatus" && $ptype=="edit")
	{
		return "catID=3";
	}
				if($table=="audit_form" && $field=="aStatus" && $ptype=="search")
	{
		return "catID=3";
	}
				if($table=="audit_form" && $field=="paid")
	{
		return "catID=8";
	}
				if($table=="audit_form" && $field=="agency")
	{
		return "catID=9";
	}
				if($table=="audit_info" && $field=="iStatus" && $ptype=="edit")
	{
		return "catID=11";
	}
				if($table=="admin_members" && $field=="seksyen")
	{
		return " CatID=4";
	}
				if($table=="admin_members" && $field=="unit")
	{
		return "catID=5";
	}
				if($table=="admin_users" && $field=="seksyen")
	{
		return " CatID=4";
	}
				if($table=="admin_users" && $field=="unit")
	{
		return "catID=5";
	}
				if($table=="auditee" && $field=="paid")
	{
		return "catID=8";
	}
				if($table=="auditee" && $field=="agency")
	{
		return "catID=9";
	}
				if($table=="web_file" && $field=="webCategory")
	{
		return "catID=13";
	}
				if($table=="reference_detail" && $field=="refCode")
	{
		return "catID=10";
	}
				if($table=="audit_form_manual" && $field=="aStatus" && $ptype=="edit")
	{
		return "catID=3";
	}
				if($table=="audit_form_manual" && $field=="aStatus" && $ptype=="add")
	{
		return "catID=3";
	}
				if($table=="audit_form_manual" && $field=="aStatus" && $ptype=="search")
	{
		return "catID=3";
	}
				if($table=="audit_form_manual" && $field=="paid")
	{
		return "catID=8";
	}
				if($table=="audit_form_manual" && $field=="agency")
	{
		return "catID=9";
	}
	return "";
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="audit_form" && $field=="aDate" && $ptype=="edit")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="audit_form" && $field=="aDate" && $ptype=="add")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="audit_form" && $field=="aStatus" && $ptype=="add")
	{
		return Draf;
	}
				if($table=="audit_form" && $field=="userID" && $ptype=="add")
	{
		return $_SESSION["userID"];
	}
				if($table=="audit_info" && $field=="iStatus" && $ptype=="add")
	{
		return "Baru";
	}
				if($table=="audit_plan" && $field=="aYear")
	{
		return strftime("%Y");
	}
				if($table=="web_file" && $field=="webCategory")
	{
		return "Minit Mesyuarat";
				...
				if($table=="web_news" && $field=="wType")
	{
		return Berita;
	}
				if($table=="web_news" && $field=="wDate")
	{
		return strftime("%Y-%m-%d");;
	}
				if($table=="web_news" && $field=="wStatus")
	{
		return "On";
	}
				if($table=="report_SKT" && $field=="aYear")
	{
		return strftime("%Y");
	}
				if($table=="web_annoucement" && $field=="wType")
	{
		return Pengumuman;
	}
				if($table=="web_annoucement" && $field=="wDate")
	{
		return strftime("%Y-%m-%d");;
	}
				if($table=="web_annoucement" && $field=="wStatus")
	{
		return "On";
	}
				if($table=="web_slider" && $field=="wType")
	{
		return Slider;
	}
				if($table=="web_slider" && $field=="wStatus")
	{
		return "On";
	}
				if($table=="web_slider" && $field=="wDate")
	{
		return strftime("%Y-%m-%d");;
	}
				if($table=="web_meeting" && $field=="webCategory")
	{
		return "Minit Mesyuarat";
	}
				if($table=="audit_form_manual" && $field=="aDate" && $ptype=="edit")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="audit_form_manual" && $field=="aDate" && $ptype=="add")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="audit_form_manual" && $field=="aStatus" && $ptype=="add")
	{
		return Draf;
				...
				if($table=="audit_form_manual" && $field=="userID" && $ptype=="add")
	{
		return $_SESSION["userID"];
	}
				if($table=="audit_form_manual" && $field=="noteAmend")
	{
		return "Daftar Query Manual";
	}
				if($table=="audit_form_manual" && $field=="noteAmend2")
	{
		return "Daftar Query Manual";
	}
	return "";
...
function runner_mail_smtp($params)
{
	...
	if ("1" == "1" && "audit@mais.gov.my"!="" || isset($params['username']))
	{
		$mail->SMTPAuth   = true;                  				// enable SMTP authentication
		$mail->Username   = isset($params['username']) ? $params['username'] : "audit@mais.gov.my";  		// SMTP username
		$mail->Password   = isset($params['password']) ? $params['password'] : "Mais1234";   // SMTP password
	}else{
		$mail->SMTPAuth = false;
	}
	if("1" == "1" && "smtp.office365.com" != "" || isset($params['host']))
	{
		$mail->Host = isset($params['host']) ? $params['host'] : "smtp.office365.com";     // sets SMTP server
	}
	else if(ini_get('SMTP') != '')
		$mail->Host = ini_get('SMTP');

	if("1" == "1" && "587" != "" || isset($params['port']))
		$mail->Port = isset($params['port']) ? $params['port'] + 0 : "587"+0;     	// set the SMTP port
