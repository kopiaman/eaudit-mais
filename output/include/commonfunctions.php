...
function checkTableName($shortTName, $type=false)
{
	...
	if ("user" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("audit_form" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("audit_info" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("set_cat" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("set_field" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("set_audit_element" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("admin_users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("auditee" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("set_mais_orga" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("audit_plan" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("all" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("web_file" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("main" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("jumlah_audit_bahagian" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("reference_detail" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("finding" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("web_news" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("report_SKT" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("report_susulan" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("web_annoucement" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("web_slider" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("web_meeting" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("set_audit_subType" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("set_audit_type" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("audit_form_manual" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$strPerm = GetUserPermissions("audit_form");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="audit_form";
	}
	$strPerm = GetUserPermissions("audit_info");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="audit_info";
	}
	$strPerm = GetUserPermissions("set_cat");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="set_cat";
	}
	$strPerm = GetUserPermissions("set_field");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="set_field";
	}
	$strPerm = GetUserPermissions("set_audit_element");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="set_audit_element";
	}
	...
	$strPerm = GetUserPermissions("auditee");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="auditee";
	}
	$strPerm = GetUserPermissions("set_mais_orga");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="set_mais_orga";
	}
	$strPerm = GetUserPermissions("audit_plan");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="audit_plan";
	}
	...
	$strPerm = GetUserPermissions("web_file");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="web_file";
	}
	...
	$strPerm = GetUserPermissions("jumlah_audit_bahagian");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="jumlah_audit_bahagian";
	}
	$strPerm = GetUserPermissions("reference_detail");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="reference_detail";
	}
	$strPerm = GetUserPermissions("finding");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="finding";
	}
	$strPerm = GetUserPermissions("web_news");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="web_news";
	}
	$strPerm = GetUserPermissions("report_SKT");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="report_SKT";
	}
	$strPerm = GetUserPermissions("report_susulan");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="report_susulan";
	}
	$strPerm = GetUserPermissions("web_annoucement");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="web_annoucement";
	}
	$strPerm = GetUserPermissions("web_slider");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="web_slider";
	}
	$strPerm = GetUserPermissions("web_meeting");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="web_meeting";
	}
	$strPerm = GetUserPermissions("set_audit_subType");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="set_audit_subType";
	}
	$strPerm = GetUserPermissions("set_audit_type");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="set_audit_type";
	}
	$strPerm = GetUserPermissions("audit_form_manual");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="audit_form_manual";
	}
	return $arr;
}
...
{
	if($shorttable=="jumlah_audit_bahagian")
		return "2DColumn";
	return "";
}
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		if(!$caseInsensitiveUsername)
		{
			$sql = "select ".$grConnection->addFieldWrappers( "GroupID" )
				...
				." from ". $grConnection->addTableWrappers( "user_ugmembers" )
	...
	$sql = "select ". $rConnection->addFieldWrappers( "TableName" )
		...
		." from ". $rConnection->addTableWrappers( "user_ugrights" )
...
{	
	...
	if(array_key_exists("user",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("audit_form",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("audit_info",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("set_cat",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("set_field",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("set_audit_element",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("admin_users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("auditee",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("set_mais_orga",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("audit_plan",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("all",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("web_file",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("main",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("jumlah_audit_bahagian",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("reference_detail",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("finding",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("web_news",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("report_SKT",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("report_susulan",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("web_annoucement",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("web_slider",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("web_meeting",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("set_audit_subType",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("set_audit_type",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("audit_form_manual",$_SESSION["UserRights"]["Guest"]))
		return true;
	return false;
}
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("user")
...
function SetLangVars($xt, $prefix, $pageName = "", $extraparams = "")
{
	...
	$xt->assign("MalayLANGLINK", "Malay" != $currentLang);
