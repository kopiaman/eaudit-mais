...
$page_layouts["finance_element_detailspreview"] = $layout;
...
if($mastertable == "audit_info")
{
	$where = "";
		$formattedValue = make_db_value("inid",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$where .= $pageObject->getFieldSQLDecrypt("inid") . "is null";
	else
		$where .= $pageObject->getFieldSQLDecrypt("inid") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	receiptNo - 
		    ...
		    $value = $viewContainer->showDBValue("receiptNo", $data, $keylink);
			$row["receiptNo_value"] = $value;
			$format = $pSet->getViewFormat("receiptNo");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("receiptNo")))
				$class = ' rnr-field-number';
			$row["receiptNo_class"] = $class;
	//	detail - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("finance_element", "detailspreview"));
